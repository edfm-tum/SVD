...
The module is configured in the [project file](project_file.qmd). In addition to the `enabled` and `type` setting, the wind module has the following settings:

#### `regionalProbabilityGrid` (filepath)
The file containing a the 10km probability of wind occurrence.

#### `transitionFile` (filepath)
The file containing a transition matrix (see [matrix module](module_matrix.qmd)) for post wind states (see above).
...
The module can provide tabular and gridded [outputs.](outputs.qmd#Wind)
