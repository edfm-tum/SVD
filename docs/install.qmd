---
title: "Installing and compiling SVD"
---
...
The `executable` folder contains all the required files and libraries for Windows. There is no separate installation. To run SVD, start the `SVDUI.exe` ([instructions](svdUI.qmd)).
...
4.  **Build Configuration:** In `SVDModel/config.pri`, uncomment the line: `DEFINES += USE_CUDA`
...
SVD can be used *without* ONNX Runtime. This version is not able to use DNNs for estimating state transitions, but can still be useful, e.g., as a pure state-and-transition-model using the [matrix module](module_matrix.qmd).
