...
The post-fire transition is defined by the transition matrix in `transitionFile`. Note that different transitions can be defined for low- and high-severity fire. High severity is encoded by `key=1` in the matrix (see [matrix module](module_matrix.qmd) for details).
...
The file containing a transition matrix (see [matrix module](module_matrix.qmd)) for post fire states (see above).
...
The module can provide tabular and gridded [output](outputs.qmd#Fire).
