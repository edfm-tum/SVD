...
| Column     | Desription                                                          |
|------------|---------------------------------------------------------------------|
| stateId    | The [Id](states.qmd) of the current state                            |
| key        | an additional qualifier (see text)                                  |
| targetId   | The [Id](states.qmd) of the target state                             |
...
The information whether a cell has shallow or deep soil, can be derived from the environment (assuming there is a variable `soilDepth`) and can be expressed as an [expression](variables.qmd) (`keyFormula`):
...
To set the module as handler for specific states, use the module name as the `type` property in the [state](states.qmd) definition (see [modules](modules.qmd)).

## Configuration

The module is configured in the [project file](project_file.qmd).
