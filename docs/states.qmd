...
The potential states are defined by the setting [`states.file`](project_file.qmd) in the project file.
...
* A list of all allowed species is defined in `model.species`. The preferred species coding scheme is a four character combination of genus and species (e.g., "*Pi*cea *ab*ies" -> "piab")
* the composition is a space-delimited string of up to four species (e.g. "PIAB fasy pisy")
* uppercase species codes (e.g. "PIAB") indicate the dominate species, lowercase species codes ("piab") specify admixed species
...
add state-specific variables (e.g. the [fire module](module_fire.qmd)).
...
that are currently in one of the handled states. See the [modules](modules.qmd) page for more details.
